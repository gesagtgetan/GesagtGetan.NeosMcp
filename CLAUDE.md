...
- **Consider removing redirect tools** — The redirect tools (`list_redirects`, `get_redirect`, `create_redirect`, `remove_redirect`) add surface area but see little practical use. Redirects are a niche concern that editors rarely need LLM help with. Removing them would simplify the tool set and reduce maintenance burden. Evaluate whether any real-world usage justifies keeping them.
