...
- **Enrich CR commands with authenticated user** — The JWT `sub` claim now contains the Neos `UserId` (UUID). `McpHttpController` already resolves it to a personal workspace. The remaining step: set the `UserId` as the initiating user on CR write commands so the event store records who made each change. No immediate consumer today, but future Neos versions may ship a content history feature where proper user attribution becomes valuable.
