...
- **Tool description quality review** — Evaluate whether the MCP tool descriptions provide enough context for LLMs to use them effectively. Claude seems to handle them well, but test with ChatGPT (and other MCP clients) to get feedback — different models may need more guidance on parameter semantics, expected workflows, or when to use which tool. Improve descriptions where needed.
