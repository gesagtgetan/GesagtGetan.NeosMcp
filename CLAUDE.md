...
- ~~**Use authenticated Neos session for CR writes**~~ — Decided against. The `llm-review` workspace is the security boundary: all CR writes are staged, nothing reaches `live` without a human publishing. Requiring a Neos session would break MCP when the session expires (30–60 min inactivity), defeating the purpose of OAuth + JWT. The `McpUser` role is the access gate — admins won't assign it to untrusted users. **Redirects are the exception**: they bypass the workspace and go live immediately. A compromised JWT could create phishing redirects for non-node paths. Existing nodes win over redirects, which limits the attack surface, but redirect mutations still deserve aggressive logging or a separate permission level.
