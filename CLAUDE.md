...
- **ChatGPT connector support** — Config-only: (1) add ChatGPT redirect URIs (`chatgpt.com/connector_platform_oauth_redirect`, `platform.openai.com/apps-manage/oauth`) to `client.knownRedirectUris`, (2) add CORS origins (`chatgpt.com`, `chat.openai.com`) to `corsAllowedOrigins`. Verify whether ChatGPT requires the RFC 8707 `resource` parameter in authorization/token requests (would need `aud` claim in JWT). No DCR needed — pre-registered client works.
- **Enrich CR commands with authenticated user** — The JWT `sub` claim contains the Neos account identifier (username/email) of the user who authorized the OAuth flow. In `McpHttpController`, resolve it to the Neos `UserId` and set it as the initiating user on CR write commands so the event store records who made each change. No immediate consumer today, but future Neos versions may ship a content history feature where proper user attribution becomes valuable.
