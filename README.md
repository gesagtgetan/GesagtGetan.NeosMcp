...
The staging-host or example.org domains require HTTP basic auth via `Web/.htaccess`. The OAuth/MCP routes (`/.well-known/oauth-*`, `/oauth/token`, `/neos/mcp`) are exempted so Claude can reach them without basic auth credentials. The authorization endpoint (`GET /neos/mcp`) is also exempted but requires a Neos session, so there is no security gap.
